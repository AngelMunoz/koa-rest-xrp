...
async function login(ctx, next) {
  ...
  if (!ctx.request.body || !ctx.request.body.email || !ctx.request.body.password) {
    ...
    return ctx.body = { error: 'Missing Fields' }
  }
  const payload = Object.assign({}, ctx.request.body);
  const creds = await User.findOne({ email: payload.email }).select('email').select('password').exec();
...
async function signup(ctx, next) {
  ...
  if (!ctx.request.body) {
    ctx.response.status = 400;
    return ctx.body = { error: 'Missing Request Body' }
  }
  const payload = Object.assign({}, ctx.request.body);
  ...
  try {
    ...
    if (error.code === 11000) {
      return ctx.body = { error: 'That Email is already in use.' }
    }
    return ctx.body = { error: 'Missing Request Body' }
  }
